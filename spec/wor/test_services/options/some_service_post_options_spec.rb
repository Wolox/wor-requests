      ...
      context 'when invalid option set for method to hash' do
        ...
        context 'without rescue' do
          it 'raises exception' do
            expect{ service.post_with_unpermitted_params }.to raise_error(Wor::Requests::InvalidOptionsError)
          end
        end

        context 'with rescue' do
          let(:error) { service.post_with_unpermitted_params_with_rescue }
          ...
          it 'returns an error with valid options' do
            expect(error.valid).to eq(Wor::Requests::Base::POST_ATTRIBUTES)
          end
